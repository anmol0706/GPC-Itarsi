...
const Downloads = () => {
  ...
  const fetchDocuments = async () => {
    try {
      ...
      console.log('Fetching documents from:', `${config.apiUrl}/api/documents`);
      const currentToken = getToken();
      console.log('Fetching documents with token:', currentToken ? 'Token exists' : 'No token');
      ...
      // Add additional properties needed for UI
      const processedDocuments = response.data.map(doc => ({
        ...doc,
        fileExists: true,
        fileIcon: doc.fileType?.includes('pdf') ? 'file-pdf' :
                 doc.fileType?.includes('word') ? 'file-word' :
                 doc.fileType?.includes('excel') ? 'file-excel' : 'file-document',
        fileExtension: doc.filePath ? doc.filePath.split('.').pop() : ''
      }));
      ...
      setError('Failed to load downloadable documents. ' + (error.response?.data?.message || error.message));
      setLoading(false);
    }
  };
                ...
                <div className="flex flex-col md:flex-row md:justify-between md:items-start">
                  ...
                  <div className="mt-4 md:mt-0 md:ml-6 flex-shrink-0">
                    {doc.type === 'drive_link' ? (
                      ...
                      <a
                        href={`${config.apiUrl}/api/download/document/${doc._id}`}
                        onClick={(e) => {
                          e.preventDefault();
                          const currentToken = getToken();
                          if (!currentToken) {
                          ...
                          // Open in new window with token in Authorization header
                          const downloadWindow = window.open('', '_blank');
                          fetch(`${config.apiUrl}/api/download/document/${doc._id}`, {
                            headers: {
                              'Authorization': `Bearer ${currentToken}`
                            }
                          })
                          ...
                          .catch(error => {
                            ...
                            downloadWindow.document.write('Download failed. Please try again.');
                          });
                        }}
                        target="_blank"
                        rel="noopener noreferrer"
