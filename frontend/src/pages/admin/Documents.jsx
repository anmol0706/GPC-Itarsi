...
const Documents = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log('Uploading document with data:', {
        ...
        hasFile: formData.file ? 'Yes' : 'No',
        driveUrl: formData.driveUrl || 'None'
      });

      const currentToken = getToken();
      console.log('Uploading document with token:', currentToken ? 'Token exists' : 'No token');

      await axios.post(`${config.apiUrl}/api/admin/documents`, data, {
        headers: {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      const response = await axios.delete(`${config.apiUrl}/api/admin/documents/${id}`, {
        headers: {
          'Authorization': `Bearer ${currentToken}`
        }
      });
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="ml-2 flex-shrink-0 flex">
                      {document.type === 'drive_link' ? (
                        ...
                        <a
                          href={`${config.apiUrl}/api/download/document/${document._id}`}
                          target="_blank"
                          rel="noopener noreferrer"
