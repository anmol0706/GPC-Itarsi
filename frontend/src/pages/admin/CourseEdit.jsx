...
const CourseEdit = () => {
  ...
  const fetchCourseDetails = async (courseId) => {
    try {
      ...
      if (course.image) {
        setImagePreview(`${axios.defaults.baseURL}/uploads/courses/${course.image}`);
      }
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isEditMode) {
        response = await axios.put(`${config.apiUrl}/api/admin/update-course/${id}`, formDataToSend, {
          headers: {
        ...
        response = await axios.post(`${config.apiUrl}/api/admin/add-course`, formDataToSend, {
          headers: {
            'Content-Type': 'multipart/form-data',
            'Authorization': `Bearer ${localStorage.getItem('token')}`
          }
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      await axios.delete(`${config.apiUrl}/api/admin/delete-course/${id}`, {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        }
      });
                ...
                <div className="flex items-center space-x-6">
                  {(imagePreview || selectedCourse?.image) && (
                    <div className="w-32 h-32 rounded-md overflow-hidden">
                      <img
                        src={imagePreview || `${config.apiUrl}/uploads/courses/${selectedCourse.image}`}
                        alt="Course Preview"
