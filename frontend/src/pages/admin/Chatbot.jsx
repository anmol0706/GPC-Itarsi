...
const Chatbot = () => {
  ...
  const fetchFaqs = async () => {
    try {
      setLoading(true);
      const response = await axios.get(`${config.apiUrl}/api/chatbot/faqs`);
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(`${config.apiUrl}/api/chatbot/faqs/${id}`, {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!question.trim() || !answer.trim()) {
      showErrorToast('Question and answer are required');
      return;
    }
    ...
    try {
      if (formMode === 'add') {
        ...
        // Update existing FAQ
        await axios.put(
          `${config.apiUrl}/api/chatbot/faqs/${currentFaqId}`,
          { question, answer, keywords: keywordsArray },
          { headers: { Authorization: `Bearer ${token}` } }
        );
      ...
      showErrorToast(`Failed to ${formMode === 'add' ? 'add' : 'update'} FAQ`);
    }
  };
  ...
  const handleTestChatbot = async (e) => {
    ...
    if (!testQuery.trim()) {
      showWarningToast('Please enter a query to test');
      return;
    }

    try {
      setTestLoading(true);
      const response = await axios.post(`${config.apiUrl}/api/chatbot/query`, { query: testQuery });
      setTestResponse(response.data);
      setTestLoading(false);
    } catch (error) {
      console.error('Error testing chatbot:', error);
      showErrorToast('Failed to test chatbot');
      setTestLoading(false);
    }
  };
