...
const StudyMaterials = () => {
    ...
    if (materials.length > 0) {
      ...
      if (selectedClass) {
        filtered = filtered.filter(material =>
          material.forClass === selectedClass || material.class === selectedClass
        );
      }
  ...
  const fetchStudyMaterials = async () => {
    try {
      ...
      // Try our new dedicated endpoint for teacher dashboard
      let materialsData = null;

      try {
        const response = await axios.get(`${config.apiUrl}/api/teacher-dashboard/study-materials`);

        if (Array.isArray(response.data)) {
          console.log('Successfully fetched study materials from teacher dashboard endpoint');
          materialsData = response.data;
        }
      } catch (error) {
        console.warn('Error fetching from teacher dashboard endpoint:', error.message);

        // Fallback to other endpoints if the new one fails
        try {
          // Try without authentication headers
          const response = await axios.get(`${config.apiUrl}/api/study-materials/all`);
          ...
          console.warn('Error fetching from fallback endpoint:', fallbackError.message);
        }
      }

      // Process the materials data if we got it
      if (materialsData) {
        const materialsWithFileStatus = materialsData.map(material => {
        ...
        console.error('Failed to fetch study materials from any endpoint');
        setMaterials([]);
  ...
  const fetchClasses = async () => {
    try {
      ...
      // Use the teacher-dashboard endpoint for students
      const response = await axios.get(`${config.apiUrl}/api/teacher-dashboard/students`, {
        headers: {
      ...
      console.error('Error fetching classes from teacher-dashboard endpoint:', error);

      // Try the old endpoint as fallback
      try {
        const fallbackResponse = await axios.get(`${config.apiUrl}/api/teachers/students`, {
          headers: {
            Authorization: `Bearer ${token}`
          }
        });
        ...
        // Extract unique classes
        const uniqueClasses = [...new Set(fallbackResponse.data.map(student => student.class))];
        setClasses(uniqueClasses);
      } catch (fallbackError) {
        console.error('Error fetching classes from fallback endpoint:', fallbackError);

        // Try to get classes from study materials if student endpoints fail
        try {
          const materialsResponse = await axios.get(`${config.apiUrl}/api/study-materials`, {
            headers: {
              Authorization: `Bearer ${token}`
            }
          });
          ...
          if (Array.isArray(materialsResponse.data)) {
            const classesFromMaterials = [...new Set(materialsResponse.data
              .filter(material => material.class || material.forClass)
              .map(material => material.class || material.forClass))];

            setClasses(classesFromMaterials);
          }
        } catch (materialError) {
          console.error('Error fetching classes from materials:', materialError);
        }
      }
    }
  };
  ...
  const handleAddMaterial = async (e) => {
    ...
    try {
      ...
      formDataToSend.append('subject', 'General'); // Default subject
      formDataToSend.append('class', formData.forClass); // Backend expects 'class' not 'forClass'
                  ...
                  <div className="px-4 py-4 sm:px-6">
                    ...
                    <div className="mt-2 sm:flex sm:justify-between">
                      ...
                      <div className="mt-2 flex items-center text-sm text-gray-500 sm:mt-0">
                        ...
                        For Class: {material.forClass || material.class}
                      </div>
                    </div>
