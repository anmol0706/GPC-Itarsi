 ...
 */
const Alert = ({ type = 'info', message, dismissible = true, onDismiss, className = '' }) => {
  ...
  return (
    <div className={`rounded-lg border p-4 mb-4 flex items-start ${style.bg} ${style.border} ${style.text} ${className}`} role="alert">
      ...
      {dismissible && onDismiss && (
        <button
          type="button"
          className={`ml-auto -mx-1.5 -my-1.5 rounded-lg focus:ring-2 p-1.5 inline-flex h-8 w-8 ${style.closeColor} hover:bg-opacity-20 hover:bg-gray-200 focus:outline-none`}
          onClick={onDismiss}
          aria-label="Close"
        >
